...
// Get completed bookings for the logged-in provider
router.get('/provider/history', auth, async(req, res) => {
    try {
        if (req.user.role !== 'provider') {
            return res.status(403).json({
                msg: 'Only providers can view booking history'
            });
        }
        ...
        console.error(
            '❌ Error fetching provider booking history:',
            err.message
        );
...
router.post('/', auth, async(req, res) => {
    try {
        ...
        const hasBookingInSameCategory = activeBookings.some(
            (b) => b.providerId && b.providerId.category === category
        );
    ...
    try {
        ...
        const bookings = await Booking.find({ userId: req.user.id })
            .populate({
                ...
                select: '_id category location contact'
            })
            .populate('userId', 'name email');
        ...
        const enriched = await Promise.all(
            bookings.map(async(booking) => {
                ...
                return {
                    ...
                    provider: {
                        _id: b.providerId && b.providerId._id,

                        name: b.providerId && b.providerId.userId ?
                            b.providerId.userId.name : "Unknown",

                        email: b.providerId && b.providerId.userId ?
                            b.providerId.userId.email : "",

                        category: b.providerId && b.providerId.category,

                        location: b.providerId && b.providerId.location,

                        contact: b.providerId && b.providerId.contact
                    },
                    hasReview
                };
            })
