...
//     const { category, lat, lng, availability, contact, image } = req.body;
...
//       location: {
//         type: "Point",
//         coordinates: [lng, lat]  // VERY IMPORTANT
//       }
//     };
...
//     // STEP 1: Geo search
//     let providers = await ServiceProvider.aggregate([
//       {
//         $geoNear: {
...
//     // STEP 2: Populate user data manually after aggregation
//     providers = await ServiceProvider.populate(providers, {
//       path: "userId",
//       select: "name email"
//     });
...
/* ========================================================
   ✅ DASHBOARD UPDATE (SAFE GEOJSON UPDATE)
======================================================== */
router.put('/dashboard/update', auth, async (req, res) => {
  try {
    if (req.user.role !== 'provider') {
    ...
    let providers = await Provider.aggregate([
      {
        $geoNear: {
          ...
          distanceField: "distance",
          spherical: true,
          maxDistance: 20000   // Increase search radius to 20km
        }
      },
      {
        $match: {
...
/* ========================================================
   ✅ GET PROVIDERS BY SERVICE (Used by non-nearby pages)
======================================================== */
router.get('/service/:serviceName', async (req, res) => {
  const { serviceName } = req.params;
